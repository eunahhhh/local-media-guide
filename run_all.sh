...
compress_file() {
  ...
  if [ "$FILESIZE" -gt "$SIZE_LIMIT" ]; then
    ...
    # 이진 탐색으로 목표 용량에 최대한 근접한 quality 탐색 (용량 무관 공통 적용)
    local ORIG_FILE="${FILE}.orig_backup"
    local TMP_FILE="${FILE}.tmp_compress"
    local Q_LOW=10 Q_HIGH=95 Q_MID BEST_FILE="${FILE}.best_compress"
    cp "$FILE" "$ORIG_FILE"   # 원본 백업 (루프 내내 보존)
    while [ $((Q_HIGH - Q_LOW)) -gt 3 ]; do
      Q_MID=$(( (Q_LOW + Q_HIGH) / 2 ))
      cp "$ORIG_FILE" "$TMP_FILE"   # 항상 원본 기준으로 테스트
      sips -s format $FORMAT -s formatOptions $Q_MID "$TMP_FILE" --out "$TMP_FILE" &>/dev/null
      local TMP_SIZE
      TMP_SIZE=$(stat -f%z "$TMP_FILE")
      if [ "$TMP_SIZE" -le "$SIZE_LIMIT" ]; then
        cp "$TMP_FILE" "$BEST_FILE"  # 조건 통과한 파일 자체를 보존 (재압축 오차 방지)
        Q_LOW=$Q_MID                 # 더 높은 quality도 가능한지 탐색
      else
        Q_HIGH=$Q_MID                # quality 낮춰야 함
      fi
    done
    if [ -f "$BEST_FILE" ]; then
      cp "$BEST_FILE" "$FILE"  # 탐색 중 가장 적합했던 파일을 최종 결과로 사용
    else
      # 한 번도 통과 못한 경우 (원본이 quality 10도 초과) → quality 10으로 강제 압축
      sips -s format $FORMAT -s formatOptions 10 "$ORIG_FILE" --out "$FILE" &>/dev/null
    fi
    rm -f "$ORIG_FILE" "$TMP_FILE" "$BEST_FILE"
    FILESIZE=$(stat -f%z "$FILE")
